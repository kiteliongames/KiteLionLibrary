...
if true, 
*/
public class PhotonArenaManager : SingletonPunCallbacks<PhotonArenaManager>
{
    ...
    public static Vector3 DefaultSpawnLocation = new(0f, 15f, 0f);
    ...
    /// </summary>
    public RoomOptions RoomOptions = new() {
        MaxPlayers = 20,
    ...
    void Awake() {
        this._fakeServer.DataStore = new Dictionary<string, object>();
        this._fakeServer.totalPlayers = 0;
    ...
    public void ConnectAndJoinRoom(string username, string[] singletons) {
        if(CurrentServerUserDepth != ServerDepthLevel.Offline) {
            KiteLionGames.BetterDebug.CBUG.Do("Redundant connect call! Already at " + CurrentServerUserDepth.ToString());
            return;
        }

        KiteLionGames.BetterDebug.CBUG.Do("Connecting!");


        PhotonNetwork.NetworkingClient.AppId = PhotonNetwork.PhotonServerSettings.AppSettings.AppIdRealtime;

        PhotonNetwork.AuthValues = new AuthenticationValues();
        PhotonNetwork.AuthValues.UserId = username + "_"+ Random.Range(0, 9999);

        string region = "us";
        bool _result = PhotonNetwork.ConnectToRegion(region);

        KiteLionGames.BetterDebug.CBUG.Log("PunCockpit:ConnectToRegion(" + region + ") -> " + _result);
    }
    ...
    public Photon.Realtime.Room GetRoom() {
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
            ...
            KiteLionGames.BetterDebug.CBUG.Error("We are not currently in a room!");
            return null; //??? TODO HANDLE BETTER!
        }
    }
    ...
    public System.Object GetData(string label) {
        ...
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
            ...
            KiteLionGames.BetterDebug.CBUG.Error("GetData only available when Offline or InRoom, this was called at " + CurrentServerUserDepth.ToString() + ".");
        }
        if(containsData == false) {
            KiteLionGames.BetterDebug.CBUG.Error("No data was found for " + label + ".");
        }
        return null;
    }
    ...
    public void SaveData(string label, System.Object data) {
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
            ...
            KiteLionGames.BetterDebug.CBUG.Error("SaveData only available when Offline or InRoom, this was called at  " + CurrentServerUserDepth.ToString() + ".");
        }
    }
    ...
    public GameObject SpawnSingletonResource(string singletonName) {
        ...
        if(singleton == null) {
            KiteLionGames.BetterDebug.CBUG.SeriousError("No singleton is available for name: " + singletonName);
            return null;
        }
    ...
    public void NewRoom() {
        ...
        KiteLionGames.BetterDebug.CBUG.Log("Leaving current room ...");
        needsNewRoom = true;
    }
    ...
    public GameObject SpawnObject(string resourceName) {
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
            ...
            KiteLionGames.BetterDebug.CBUG.Error("SpawnObject only available when Offline or InRoom, this was called at " + CurrentServerUserDepth.ToString() + ".");
            return null;
        }
    }
    ...
    public string GetLocalUsername() {
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
            ...
            KiteLionGames.BetterDebug.CBUG.Error("Username only available when Offline or InRoom, this was called at " + CurrentServerUserDepth.ToString() + ".");
            return null;
        }
    }

    /// <summary>
    /// Not relevant until next room join.
    /// </summary>
    /// <param name="UID">Unique ID (PUN Game Version) </param>
    public void DefineUniqueSession(string UID)
    {
        PhotonNetwork.GameVersion = UID;
    }
    ...
    public GameObject SpawnPlayer(Vector3 pos, Quaternion rot, string ResourceName="PhotonArenaPlayer") {
        if (CurrentServerUserDepth == ServerDepthLevel.Offline) {
           ...
           return PhotonNetwork.Instantiate(ResourceName, pos, rot);
            ...
            KiteLionGames.BetterDebug.CBUG.Error("Spawn Player only available when Offline or InRoom, this was called at " + CurrentServerUserDepth.ToString() + ".");
        }
        return null;
    }
...
//        } else {
//            KiteLionGames.BetterDebug.CBUG.Error("This can only be called when player is in a room or offline! You're currently in: " + CurrentServerUserDepth.ToString());
//            return false;
//        }
//    }
    ...
    public override void OnConnected() {
        base.OnConnected();
        KiteLionGames.BetterDebug.CBUG.Do("Connected!");
    ...
    public override void OnConnectedToMaster() {
        ...
        KiteLionGames.BetterDebug.CBUG.Do("Connected To Master! Joining Lobby ...");
        TypedLobby sqlLobby = new("Lobby", LobbyType.SqlLobby);
        bool success = PhotonNetwork.JoinLobby(sqlLobby);

        if (!success) {
            KiteLionGames.BetterDebug.CBUG.Do("PunCockpit: Could not join Lobby ...");
        }
    }
    ...
    public override void OnJoinedLobby() {
        KiteLionGames.BetterDebug.CBUG.Log("Lobby Joined!");
        KiteLionGames.BetterDebug.CBUG.Log("Joining Random Room ...");
        if(needsNewRoom) {
    ...
    public override void OnDisconnected(DisconnectCause cause) {
        KiteLionGames.BetterDebug.CBUG.Log("OnDisconnected(" + cause + ")");
    }

    //public override void On

    public override void OnJoinedRoom() {
        KiteLionGames.BetterDebug.CBUG.Log("Joined Room! Total Players: " + PhotonNetwork.CurrentRoom.PlayerCount);
    ...
    public override void OnJoinRandomFailed(short returnCode, string message) {
        KiteLionGames.BetterDebug.CBUG.Log("Room Join failed. Creating a room ...");
        PhotonNetwork.CreateRoom(null, RoomOptions, null);
    }

    public override void OnCreateRoomFailed(short returnCode, string message)
    {
        KiteLionGames.BetterDebug.CBUG.Do($"Create Room Failed! {message}, Return Code: {returnCode}");
    }
    #endregion
}
